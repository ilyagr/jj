...
#[test]
fn test_copy_diffstream_chain_rename() -> TestResult {
    ...
    let a_copyid = repo.store().backend().write_copy(&a_history).block_on()?;
    ...
    let c_copyid = repo.store().backend().write_copy(&c_history).block_on()?;

    let left = create_tree_with_copy_id(
        repo,
        &[
            (a, "a content", &a_copyid),
            (b, "b content", &b_copyid_orig),
        ],
    );
    let a_val = left.path_value(a).block_on()?;
    let old_b_val = left.path_value(b).block_on()?;
    let right = create_tree_with_copy_id(
        repo,
        &[(b, "was a", &b_copyid_renamed), (c, "was b", &c_copyid)],
    );
    let new_b_val = right.path_value(b).block_on()?;
    let c_val = right.path_value(c).block_on()?;
...
#[test]
fn test_copy_diffstream_copy_over_symlink() -> TestResult {
    ...
    let foo_val = left.path_value(foo).block_on()?;
    let symlink_val = left.path_value(bar).block_on()?;
