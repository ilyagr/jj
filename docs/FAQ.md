...
the `--preserve-descendant-contents` flag so that the *file contents* (AKA snapshot) of
the "featureB" change is preserved.

```console
$ # We refer to `lnvvtr` as `@-` for brevity
$ jj restore --from b80 --into @- --preserve-descendant-contents
...
Even though `@-` was modified, `--preserve-descendant-contents` preserved the contents of
the current change:
...
??? info "More details on what `--preserve-descendant-contents` does"
