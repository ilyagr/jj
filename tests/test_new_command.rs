...
fn test_new_insert_after() {
    ...
    Parent commit      : 6041917ceeb5 (empty) B
    Parent commit      : c9257eff5bf9 (empty) D
    "###);
    insta::assert_snapshot!(get_short_log_output(&test_env, &repo_path), @r###"
    ◉  C
    │ ◉  F
...
fn test_new_insert_after_children() {
    ...
    Parent commit      : 65b1ef43c737 (empty) A
    Parent commit      : ec18c57d72d8 (empty) C
    "###);
    insta::assert_snapshot!(get_short_log_output(&test_env, &repo_path), @r###"
    @    G
    ├─╮
...
fn test_new_insert_before() {
    ...
    Parent commit      : 41a89ffcbba2 (empty) E
    Parent commit      : c9257eff5bf9 (empty) D
    Parent commit      : 6041917ceeb5 (empty) B
    "###);
    insta::assert_snapshot!(get_short_log_output(&test_env, &repo_path), @r###"
