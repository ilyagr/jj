...
fn test_new_insert_after() {
    ...
    Working copy now at: ca7c6481a8dd (empty) G
    Parent commit      : 6041917ceeb5 (empty) B |B
    Parent commit      : c9257eff5bf9 (empty) D |D
    "###);
    insta::assert_snapshot!(get_short_log_output(&test_env, &repo_path), @r###"
    ◉  C
    │ ◉  F
    ...
    Working copy now at: fcf8281b4135 (empty) H
    Parent commit      : c9257eff5bf9 (empty) D |D
    "###);
    insta::assert_snapshot!(get_short_log_output(&test_env, &repo_path), @r###"
    ◉  C
...
fn test_new_insert_after_children() {
    ...
    insta::assert_snapshot!(stdout, @r###"
    Working copy now at: b48d4d73a39c (empty) G
    Parent commit      : 65b1ef43c737 (empty) A |A
    Parent commit      : ec18c57d72d8 (empty) C |C
    "###);
    insta::assert_snapshot!(get_short_log_output(&test_env, &repo_path), @r###"
    @    G
    ├─╮
...
fn test_new_insert_before() {
    ...
    insta::assert_snapshot!(stdout, @r###"
    Rebased 2 descendant commits
    Working copy now at: ff6bbbc7b8df (empty) G
    Parent commit      : 41a89ffcbba2 (empty) E |E
    Parent commit      : c9257eff5bf9 (empty) D |D
    Parent commit      : 6041917ceeb5 (empty) B |B
    "###);
    insta::assert_snapshot!(get_short_log_output(&test_env, &repo_path), @r###"
...
fn test_new_insert_before_root_successors() {
    ...
    Working copy now at: 3654197754f8 (empty) G
...
fn test_new_insert_before_no_root_merge() {
    ...
    Working copy now at: bf9fc49331de (empty) G
    Parent commit      : 65b1ef43c737 (empty) A |A
    "###);
    insta::assert_snapshot!(get_short_log_output(&test_env, &repo_path), @r###"
    ◉    F
