...
fn test_describe() {
    ...
    Working copy now at: cf3e86731c67 (empty) description from CLI
    ...
    let stdout = test_env.jj_cmd_success(&repo_path, &["describe"]);
    insta::assert_snapshot!(stdout, @r###"
    Working copy now at: 100943aeee3f (empty) description from editor
    Parent commit      : 000000000000 (empty) (no description set)
    "###);
    ...
    let stdout = test_env.jj_cmd_success(&repo_path, &["describe"]);
    insta::assert_snapshot!(stdout, @r###"
    Working copy now at: ccefa58bef47 (empty) description among comment
    Parent commit      : 000000000000 (empty) (no description set)
    "###);
    ...
    let stdout = test_env.jj_cmd_success(&repo_path, &["describe"]);
    insta::assert_snapshot!(stdout, @r###"
    Working copy now at: e932ba42cef0 (empty) line1
    Parent commit      : 000000000000 (empty) (no description set)
    "###);
    let stdout =
    ...
    insta::assert_snapshot!(stdout, @r###"
    Working copy now at: d6957294acdc (empty) (no description set)
    Parent commit      : 000000000000 (empty) (no description set)
    "###);
    std::fs::write(&edit_script, "write\n").unwrap();
    let stdout = test_env.jj_cmd_success(&repo_path, &["describe"]);
