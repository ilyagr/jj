...
fn test_git_push_undo() {
    ...
    main: 8c05de152218 (empty) BB |main*
      @origin (ahead by 1 commits, behind by 1 commits): 0cffb6146141 (empty) AA |main@origin
    "###);
    ...
    insta::assert_snapshot!(get_branch_output(&test_env, &repo_path), @r###"
    main (conflicted):
      - 0cffb6146141 (empty) AA
      + 0a3e99f08a48 (empty) CC |main??
      + 8c05de152218 (empty) BB |main?? main@origin
      @origin (behind by 1 commits): 8c05de152218 (empty) BB |main?? main@origin
    "###);
}
...
fn test_git_push_undo_with_import() {
    ...
    insta::assert_snapshot!(get_branch_output(&test_env, &repo_path), @r###"
    main: 0a3e99f08a48 (empty) CC |main*
      @origin (ahead by 1 commits, behind by 1 commits): 8c05de152218 (empty) BB |main@origin
    "###);
}
...
fn test_git_push_undo_colocated() {
    ...
    main (conflicted):
      - 0cffb6146141 (empty) AA
      + 0a3e99f08a48 (empty) CC |main?? main@git
      + 8c05de152218 (empty) BB |main?? main@origin
      @git (behind by 1 commits): 0a3e99f08a48 (empty) CC |main?? main@git
      @origin (behind by 1 commits): 8c05de152218 (empty) BB |main?? main@origin
    "###);
}
...
fn test_git_push_undo_repo_only() {
    ...
    test_env.jj_cmd_success(&repo_path, &["git", "push"]);
    insta::assert_snapshot!(get_branch_output(&test_env, &repo_path), @r###"
    main: 0cffb6146141 (empty) AA |main
    "###);
    test_env.advance_test_rng_seed_to_multiple_of(100_000);
    test_env.jj_cmd_success(&repo_path, &["describe", "-m", "BB"]);
    insta::assert_snapshot!(get_branch_output(&test_env, &repo_path), @r###"
