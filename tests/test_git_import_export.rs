...
fn test_resolution_of_git_tracking_branches() {
    ...
    main: 3af370264cdc (empty) new_message |main*
      @git (ahead by 1 commits, behind by 1 commits): 16d541ca40f4 (empty) old_message |main@git
    "###);
...
fn test_git_export_undo() {
    ...
    insta::assert_snapshot!(get_branch_output(&test_env, &repo_path), @r###"
    a: 230dd059e1b0 (empty) (no description set) |a
    "###);
...
fn test_git_import_undo() {
    ...
    insta::assert_snapshot!(get_branch_output(&test_env, &repo_path), @r###"
    a (forgotten)
      @git: 230dd059e1b0 (empty) (no description set) |a@git
...
fn test_git_import_move_export_with_default_undo() {
    ...
    insta::assert_snapshot!(get_branch_output(&test_env, &repo_path), @r###"
    a: 096dc80da670 (empty) (no description set) |a*
      @git (behind by 1 commits): 230dd059e1b0 (empty) (no description set) |a@git
    "###);
    insta::assert_snapshot!(test_env.jj_cmd_success(&repo_path, &["git", "export"]), @"");
    insta::assert_snapshot!(get_branch_output(&test_env, &repo_path), @r###"
    a: 096dc80da670 (empty) (no description set) |a
    ...
    Working copy now at: 230dd059e1b0 (empty) (no description set)
