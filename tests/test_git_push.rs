...
fn test_git_push_current_branch() {
    ...
    insta::assert_snapshot!(stdout, @r###"
    branch1: 19e00bf64429 (empty) modified branch1 commit |branch1*
      @origin (ahead by 1 commits, behind by 1 commits): 45a3aa29e907 (empty) description 1 |branch1@origin
    branch2: 10ee3363b259 (empty) foo |branch2* my-branch
      @origin (behind by 1 commits): 8476341eb395 (empty) description 2 |branch2@origin
    my-branch: 10ee3363b259 (empty) foo |branch2* my-branch
    "###);
...
fn test_git_push_multiple() {
    ...
    insta::assert_snapshot!(stdout, @r###"
    branch1 (deleted)
      @origin: 45a3aa29e907 (empty) description 1 |branch1@origin
      (this branch will be *deleted permanently* on the remote on the
       next `jj git push`. Use `jj branch forget` to prevent this)
    branch2: 15dcdaa4f12f (empty) foo |branch2* my-branch
      @origin (ahead by 1 commits, behind by 1 commits): 8476341eb395 (empty) description 2 |branch2@origin
    my-branch: 15dcdaa4f12f (empty) foo |branch2* my-branch
    "###);
    // First dry-run
...
fn test_git_push_conflicting_branches() {
    ...
    branch1: 45a3aa29e907 (empty) description 1 |branch1
    branch2 (conflicted):
      + 8e670e2d47e1 (empty) description 3 |branch2??
      + 8476341eb395 (empty) description 2 |branch2?? branch2@origin
      @origin (behind by 1 commits): 8476341eb395 (empty) description 2 |branch2?? branch2@origin
    "###);
