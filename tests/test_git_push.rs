...
fn test_git_push_current_branch() {
    ...
    insta::assert_snapshot!(stdout, @r###"
    branch1: 19e00bf64429 (empty) modified branch1 commit
      @origin (ahead by 1 commits, behind by 1 commits): 45a3aa29e907 (empty) description 1
    branch2: 10ee3363b259 (empty) foo
      @origin (behind by 1 commits): 8476341eb395 (empty) description 2
    my-branch: 10ee3363b259 (empty) foo
    "###);
...
fn test_git_push_multiple() {
    ...
    insta::assert_snapshot!(stdout, @r###"
    branch1 (deleted)
      @origin: 45a3aa29e907 (empty) description 1
      (this branch will be *deleted permanently* on the remote on the
       next `jj git push`. Use `jj branch forget` to prevent this)
    branch2: 15dcdaa4f12f (empty) foo
      @origin (ahead by 1 commits, behind by 1 commits): 8476341eb395 (empty) description 2
    my-branch: 15dcdaa4f12f (empty) foo
    "###);
    // First dry-run
...
fn test_git_push_conflicting_branches() {
    ...
    branch1: 45a3aa29e907 (empty) description 1
    branch2 (conflicted):
      + 8e670e2d47e1 (empty) description 3
      + 8476341eb395 (empty) description 2
      @origin (behind by 1 commits): 8476341eb395 (empty) description 2
    "###);
