...
fn test_branch_delete_glob() {
    ...
    bar-2: 6fbf398c2d59 (empty) commit
    foo-1 (deleted)
      @origin: 6fbf398c2d59 (empty) commit
...
fn test_branch_forget_export() {
    ...
    insta::assert_snapshot!(stdout, @r###"
    foo: 65b6b74e0897 (empty) (no description set)
    "###);
    ...
    insta::assert_snapshot!(stdout, @r###"
    foo (forgotten)
      @git: 65b6b74e0897 (empty) (no description set)
...
fn test_branch_forget_fetched_branch() {
    ...
    insta::assert_snapshot!(stdout, @"");
    insta::assert_snapshot!(get_branch_output(&test_env, &repo_path), @r###"
    feature1: 38aefb173976 (empty) another message
    "###);
}
...
fn test_branch_list_filtered_by_revset() {
    ...
    local-keep: c7b4c09cd77c (empty) local-keep
    remote-delete (deleted)
      @origin: dad5f298ca57 (empty) remote-delete
      (this branch will be *deleted permanently* on the remote on the
       next `jj git push`. Use `jj branch forget` to prevent this)
    remote-keep: 911e912015fb (empty) remote-keep
    remote-rewrite: e31634b64294 (empty) rewritten
      @origin (ahead by 1 commits, behind by 1 commits): 3e9a5af6ef15 (empty) remote-rewrite
    "###);
