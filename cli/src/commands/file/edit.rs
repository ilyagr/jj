...
/// Edit a single file in a revison
///
/// Currently, text files and textual conflicts are supported. The revision will
/// not be updated if the editor edits with an error.
...
pub(crate) struct FileEditArgs {
    ...
    path: Vec<String>,
    // TODO: `--conflict` to make `jj file edit` find a conflict to resolve
    // (possible in a fileset), similarly to `jj resolve` TODO: Abandon edit if
    // the file is empty, add a `--make-empty` option instead to allow making a
    // file empty intentionally. We might then also want an `--allow-empty`
    // option for scripting, perhaps. TODO: Consider allowing editing symlinks,
    // converting symlinks to text. Would probably also want a `jj file ln`
    // command.
}

#[instrument(skip_all)]
pub(crate) fn cmd_file_edit(
    ui: &mut Ui,
    ...
    let fileset_expression = workspace_command.parse_file_patterns(ui, &args.path)?;
