...
/// Commands for working with the toy backend (used for testing)
#[derive(Subcommand, Clone, Debug)]
pub enum ToyBackendCommand {
    Init(ToyBackendInitArgs),
}

pub fn cmd_toy_backend(
    ui: &mut Ui,
    command: &CommandHelper,
    subcommand: &ToyBackendCommand,
) -> Result<(), CommandError> {
    match subcommand {
        ToyBackendCommand::Init(args) => cmd_toy_backend_init(ui, command, args),
    }
}
...
pub(crate) struct ToyBackendInitArgs {
...
#[instrument(skip_all)]
pub(crate) fn cmd_toy_backend_init(
    ui: &mut Ui,
    command: &CommandHelper,
    args: &ToyBackendInitArgs,
) -> Result<(), CommandError> {
