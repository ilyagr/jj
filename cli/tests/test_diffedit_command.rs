...
fn test_diffedit_restore_descendants() {
    ...
    let output = work_dir.run_jj(["diffedit", "-r", "@-", "--preserve-descendant-contents"]);
    insta::assert_snapshot!(output, @r"
    ------- stderr -------
    ...
    insta::assert_snapshot!(output, @r"
    ------- stderr -------
    Warning: `--restore-descendants` has been renamed to `--preserve-descendant-contents` AKA `--pdc`. The old name will become a hard error in the future.
